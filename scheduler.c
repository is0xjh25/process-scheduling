...
proc* arrived_insert(proc *head, proc *new_process) {
    ...
    while (current) {    
        // depends on process arrived time
...
proc* str_insert(proc *head, proc *new_process) {
    ...
    while (current) {    
        // be ordered by remaining execution time (short->long)
...
// priority list for challenge tasks
proc* advanced_insert(proc *head, proc *new_process) {
    ...
    while (current) {    
        // be ordered by remaining execution time (long->short)
...
proc** store_processing(sched *sch, proc **temp) { 
...
sched* init_scheduler(FILE *file, int num_processor, int advanced_mode) {
    ...
    while (getline(&line, &line_buffer_length, file) != EOF) {
        ...
        if (advanced_mode == FALSE) {
		    sch->pending = arrived_insert(sch->pending, process);
        } else if (advanced_mode == TRUE) {
            sch->pending = advanced_insert(sch->pending, process);
        }
...
// calculate the time need for a processor finish all its current processes
...
void default_processor_scheduler(FILE *file, int num_processor, 
    int advanced_mode) {  
    // initialization
    sched *sch = init_scheduler(file, num_processor, advanced_mode);
    // for storing current processes
    proc **temp = create_processors(num_processor);

    while (sch->init_process_remaining > sch->process_finished) {
        
        // store current processing processes
        temp = store_processing(sch, temp);
        ...
        // depends on num_processor, the scheduler would act different
                            ...
                            if (current->is_parallelisable == 
                                ...
                                for (int i = 0; i < k; i++) {
                                    ...
                                    for (int j = 0; j < num_processor; j++) {
                                        if (remaining_time(sch->processors[j])
                                            < min_time && !children[j]) {
                                            min_time
                                            = remaining_time(sch->
                                            processors[j]);
                                    ...
                                    // put child process into a processor
                                    children[index] = TRUE;
        ...
        // check new process is going to be executed
                    ...
                    if (current->time_arrived <= sch->current_time) {       
                        if (current->is_parallelisable == NON_PARALLELISABLE) {
                            ...
                            if (!previous) {
                                ...
                                parent->link = NULL;
                                sch->finished = 
                                str_insert(sch->finished,parent);
